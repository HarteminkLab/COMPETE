/* ----------------------------------------------------------------------------
   ...
   Copyright (C) 2005-2006  Mark A Lindner
...
#define CONFIG_LOOKUP_NO_EXCEPTIONS(P, V)       \
  try                                           \
  {                                             \
    Setting &s = lookup(P);                     \
    V = s;                                      \
...
{
  CONFIG_LOOKUP_NO_EXCEPTIONS(path, value);
}
...
void Setting::setFormat(SettingFormat format) throw()
{
  if(_type == TypeInt)
  {
    if(format == FormatHex)
...
Setting::operator unsigned long() const throw(SettingTypeException)
{
  ...
  if((v < 0) || (v > static_cast<long>(ULONG_MAX)))
    v = 0;
...
Setting::operator unsigned int() const throw(SettingTypeException)
{
  ...
  long v = config_setting_get_int(_setting);
  if((v < 0) || (v > static_cast<long>(UINT_MAX)))
    v = 0;
...
const std::string &Setting::operator=(const std::string &value)
...
#define SETTING_LOOKUP_NO_EXCEPTIONS(K, V) \
  try                                      \
  {                                        \
...
{
  SETTING_LOOKUP_NO_EXCEPTIONS(name, value);
}
